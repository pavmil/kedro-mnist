...
Kedro грузит конфиги через ConfigLoader, который мёржит два слоя:

- **`conf/base/`** — общий конфиг проекта: коммитится в git и шарится между
  всеми участниками (catalog, parameters, mlflow). Креды и секреты сюда класть
  нельзя.
- **`conf/local/`** — оверрайды под конкретную машину/юзера: креды и секретные
  ключи, локальные пути, IDE-настройки. Папка в `.gitignore` и в репозиторий не
  попадает. `local` — окружение запуска Kedro по умолчанию: его значения
  накатываются поверх `base`.
